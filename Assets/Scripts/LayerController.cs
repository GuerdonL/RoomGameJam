...
public class LayerController : MonoBehaviour
{
   ...
   // public GameObject floor2;
    ...
    public int switchesFlipped = 0;
    public void BossLevel(){
        Debug.Log(switchesFlipped);
        if(switchesFlipped==3){
            gameObject.GetComponent<NextScene>().LoadScene("BossArena");
            /*changing = true;
            player.GetComponent<Player>().enabled =false;
            newLayer = boss;
            newLayerCollider = bossCol;                                     
    ...
    private void ChangeOpacity(List<int> indeces, GameObject layer){
        foreach(int index in indeces){
                    ...
                    float correctedPos = (layer.transform.localPosition.y*dimmingFactor)+1;
        ...
        if(changing){
            ...
            if(!(oldLayer.transform.localPosition.y<=-1)){
                ...
                if(positionY<-1){
                    ...
                    //set rb and r to (0,-1,-5)... maybe not -5 yet?
                    newTransitionLayer.transform.localPosition=new Vector3(0,-1,0);
                    newLayer.transform.localPosition = new Vector3(0,-1,0);
